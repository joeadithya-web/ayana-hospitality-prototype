...
export function Search() {
  ...
  const results = useMemo(
    ...
    [hotels, city, stars, segments, maxPrice],
  );
  ...
  return (
    <TravellerShell active="search">
      <PageHeader title="Search Hotels" subtitle={`${results.length} properties available`} />
      ...
      <div className="flex flex-col gap-3 px-5 py-5">
        {results.map((hotel) => (
          <Card key={hotel.id} className="cursor-pointer overflow-hidden !p-0" onClick={() => navigate(`/traveller/hotel/${hotel.id}`)}>
        ...
        {results.length === 0 && <p className="py-10 text-center text-sm text-ink-700/50">No hotels match these filters.</p>}
      </div>
    </TravellerShell>
  );
}
